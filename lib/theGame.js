...
$(function () {
    ...
    if (previouslySavedState) {
        output('\n--------------------------------------PREVIOUS GAME AVAILABLE!--------------------------------------', 'alert-text');
...
if (!give)
    throw new Error('expect adventure to have give commmand');
adventure.newCommand({
    methodName: "putInto",
...
var soup = adventure.newItem({
    ...
    beExaminedBy: function (subject) {
        if (!subject.has(this)) {
            tell(subject, "It just looks like a can of soup from here.  Maybe you can examine it more closely if you pick it up.");
            return;
        }
    ...
    beOpenedBy: function (subject, instrument) {
        ...
        this.closed = false;
        tell(subject, "You successfully use " + instrumentName + " to open the can of soup.", function (witness) {
...
var bed = adventure.newItem({
    ...
    beUsedBy: function (subject) {
        tell(subject, "You lie down on the bed and fall asleep for a few minutes.  You wake up feeling refreshed.", function (witness) {
            return A.capitalize(witness.nameFor(subject)) + ' ' + subject.verb('lie') + ' down on the bed and ' +
                subject.verb('sleep') + ' for a few minutes and then ' + subject.verb('get') + ' up.';
        });
        return;
    },
    location: bedroom
...
var teddyBear = adventure.newItem({
    ...
    beExaminedBy: function (subject) {
        if (!subject.has(teddyBear)) {
            tell(subject, 'The teddy bear is brown and wearing green overalls. You can\'t tell anything else about it from here.');
            return;
        }
...
var cabinetBeOpenedBy = function (subject, instrument) {
    if (!cabinet.closed) {
    ...
    tell(subject, "You turn the key in the cabinet's lock until you hear a click.  The cabinet springs open!", function (witness) {
        return A.capitalize(witness.nameFor(subject)) + ' ' + subject.verb('open') + ' the cabinet with ' +
            witness.nameFor(key) + '.';
    });
    return;
};
var cabinet = adventure.newItem({
    id: 'cabinet',
    keywords: ['cabinet'],
    canBeTaken: false,
    beExaminedBy: function (subject) {
    ...
    beAskedToTake: function (item, subject, doTell) {
        if (!cabinet.closed) {
            var info = function (witness) {
                return A.capitalize(witness.nameFor(subject)) + ' ' + subject.verb('has') + ' put ' + witness.nameFor(item) +
                    ' into the cabinet.';
            };
            if (doTell)
        ...
        if (doTell)
            tell(subject, "You can't do that.  The cabinet is closed.");
        return false;
    },
...
var boulderBeMovedBy = function (subject) {
    var ret = "";
    if (!boulder.budged) {
        boulder.budged = true;
...
var cage = adventure.newItem({
    ...
    beAskedToTake: function (item, subject, doTell) {
        if (!this.closed) {
            if (doTell) {
                var info = function (witness) {
                    return A.capitalize(witness.nameFor(subject)) + ' ' + subject.verb('have') + ' put ' + witness.nameFor(item) + ' into ' + witness.nameFor(cage) + '.';
                };
                tell(subject, info, info);
            }
            return true;
        ...
        if (doTell)
            tell(subject, "You can't put " + subject.nameFor(item) + " into the closed cage.");
        return false;
    },
    beAskedToGive: function (item, subject, doTell) {
        ...
        if (doTell)
            tell(subject, "You can't take anything out of the cage when it is closed.");
        return false;
    }
}, {
    closed: true
