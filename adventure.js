...
(function(A) {
  ...
  var Adventure = (function() {
    function Adventure() {
      ...
      a.newDirection({
        id: 'up',
        keywords: ['up', 'u', 'upper', 'upward', 'upwards', 'above', 'high', 'higher', 'highest', 'over',
          'top'
        ],
        opposite: {
          id: 'down',
          keywords: ['down', 'd', 'lower', 'downward', 'downwards', 'below', 'lower', 'low', 'lowest',
            'under',
            'beneath',
            'underneath',
            'bottom'
        ...
        command: function help() {
          ...
          ret += commands.filter(function(c) {
            ...
            return '"' + c.templates[0].toLowerCase().split(/\s+/).map(function(x) {
              return x.startsWith('%i') ? '[ITEM]' : x.startsWith('%d') ? '[DIRECTION]' :
                capitalize(x);
    ...
    Adventure.prototype.tell = function(personOrPeople, infoPeople, infoNearby, infoDistant) {
        ...
        command.templates.forEach(function(template) {
          ...
          while (true) {
            ...
            var func = function(str, offset) {
              var ret = (typeof lens[i] === 'number') ? ('\\b(.{1,' + lens[i] + '})\\b') :
                '\\b(.+)\\b';
              i++;
              return ret;
            };
  ...
  var Exit = (function(_super) {
    ...
    function Exit(adventure, options) {
      ...
      if (reverse !== false) {
        ...
        reverse.direction = immutable(reverse.direction || (adventure.directions[unwrap(options.direction)] || {})
          .oppositeId);
      ...
      var o = {
        ...
        get: function() {
          ...
          if (typeof destination === 'string')
            ret = adventure.itemMap[destination];
          if (!ret)
            ret = null;
          return ret;
        }
